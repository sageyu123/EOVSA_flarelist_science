...
# flare_data = pd.read_csv(os.path.join(data_directory, 'EOVSA_STIX_joint_flarelist.csv'))
flare_data = pd.read_csv(os.path.join(data_directory, 'EOVSA_STIX_common_flarelist.csv'))

# Filter rows based on certain conditions
filtered_flares = flare_data[flare_data['STIX Coverage'].str.contains("yes", case=False, na=False)]
filtered_flares = filtered_flares[filtered_flares['HEK_flare_class'] != '??']
filtered_flares.drop_duplicates(inplace=True)

# Replace 'GOES Class' with 'HEK_flare_class' where necessary
condition = filtered_flares['HEK_flare_class'].notnull() & filtered_flares['HEK_flare_class'].str.strip().ne('')
filtered_flares.loc[condition, 'GOES Class'] = filtered_flares.loc[condition, 'HEK_flare_class']

# Drop irrelevant rows
drop_condition = (filtered_flares['HEK_flare_hpc_x'] == 0) & pd.isnull(filtered_flares['hgs_lon'])
filtered_flares = filtered_flares[~drop_condition]

# Convert flare classes to linear scale
filtered_flares['GOES_fl_scls'] = filtered_flares['GOES Class'].apply(flare_class_to_linear_scale)
...
filtered_flares['numeric_class'] = filtered_flares['GOES Class'].map(flare_class_to_numeric)

filtered_flares = filtered_flares.dropna(subset=['numeric_class'])

# Filter out rows with no numeric class
# filtered_numeric_classes = filtered_flares['numeric_class'].dropna()
filtered_numeric_classes = filtered_flares['numeric_class']

# Extract relevant columns for plotting
goes_class= filtered_flares['GOES Class'].values

flare_peak_times = Time(list(filtered_flares['HEK_flare_tpeak'].values))
hpc_x, hpc_y = filtered_flares['HEK_flare_hpc_x'], filtered_flares['HEK_flare_hpc_y']
hgs_lon, hgs_lat = filtered_flares['hgs_lon'], filtered_flares['hgs_lat']
earth_lon, earth_lat, earth_dist = filtered_flares['earth_position_lon'], filtered_flares['earth_position_lat'], \
    filtered_flares['earth_position_AU_distance']
...
# flare_coords = []
# sli = slice(0,2)
# for obstime, lon, lat, x, y, e_lon, e_lat, e_dist in zip(flare_peak_times[sli], hgs_lon[sli], hgs_lat[sli], hpc_x[sli],
#                                                          hpc_y[sli], earth_lon[sli],
#                                                          earth_lat[sli], earth_dist[sli]):
#     dsun_obs = (e_dist * u.AU).to(u.m).value
#     metadata = MetaDict({
#         'ctype1': 'HPLN-TAN', 'ctype2': 'HPLT-TAN',
#         'cunit1': 'arcsec', 'cunit2': 'arcsec',
#         'crpix1': (dummy_data.shape[0] + 1) / 2., 'crpix2': (dummy_data.shape[1] + 1) / 2.,
#         'cdelt1': 1.0, 'cdelt2': 1.0, 'crval1': 0.0, 'crval2': 0.0,
#         'hgln_obs': e_lon,  ## Stonyhurst heliographic longitude in degree
#         'hglt_obs': e_lat,  ## Stonyhurst heliographic latitude in degree
#         'dsun_obs': dsun_obs, 'dsun_ref': const.au.to(u.m).value,
#         'rsun_ref': const.R_sun.to(u.m).value,
#         'rsun_obs': ((const.R_sun / const.au / e_dist).decompose() * u.radian).to(u.arcsec).value,
#         't_obs': obstime.iso, 'date-obs': obstime.iso,
#     })
#     dummy_map = smap.GenericMap(dummy_data, metadata)
#
#     if pd.notnull(lon) and pd.notnull(lat):
#         flare_coords.append(
#             SkyCoord(lon=lon * u.deg, lat=lat * u.deg, frame='heliographic_stonyhurst',
#                      obstime=obstime).transform_to(dummy_map_ref.coordinate_frame))
#     else:
#         flare_coords.append(SkyCoord(Tx=x * u.arcsec, Ty=y * u.arcsec,
#                                      frame=dummy_map.coordinate_frame, distance=dsun_obs).transform_to('heliographic_stonyhurst').transform_to(dummy_map_ref.coordinate_frame))
#
#     # print(lon, lat, x, y, flare_coords[-1])
# flare_coords = SkyCoord(flare_coords)
...
# hgs_lon_new = []
# hgs_lat_new = []
# for idx,(obstime, lon, lat, x, y, e_lon, e_lat, e_dist) in enumerate(zip(flare_peak_times, hgs_lon, hgs_lat, hpc_x,
#                                                          hpc_y, earth_lon,
#                                                          earth_lat, earth_dist)):
#     dsun_obs = (e_dist * u.AU).to(u.m).value
#     metadata = MetaDict({
#         'ctype1': 'HPLN-TAN', 'ctype2': 'HPLT-TAN',
#         'cunit1': 'arcsec', 'cunit2': 'arcsec',
...
#
#     if pd.notnull(lon) and pd.notnull(lat):
#         # hgs_lon_new.append(lon)
#         # hgs_lat_new.append(lat)
#         # pass
#         coord = SkyCoord(lon=lon * u.deg, lat=lat * u.deg, frame='heliographic_stonyhurst', observer='earth',
#                  obstime = obstime).transform_to(frames.Helioprojective)
#
#     else:
#         coord = SkyCoord(Tx=x * u.arcsec, Ty=y * u.arcsec,obstime=obstime,
#                          observer='earth', frame=frames.Helioprojective)
#                                      # frame=dummy_map.coordinate_frame)
#         hgs_lon_new.append(coord.heliographic_stonyhurst.lon.value)
#         hgs_lat_new.append(coord.heliographic_stonyhurst.lat.value)
#         # flare_coords.append(SkyCoord(Tx=x * u.arcsec, Ty=y * u.arcsec,
#         #                              frame=dummy_map.coordinate_frame, distance=dsun_obs).transform_to('heliographic_stonyhurst').transform_to(dummy_map_ref.coordinate_frame))
#
#     # print(lon, lat, x, y, flare_coords[-1])
# # flare_coords = SkyCoord(flare_coords)
# flare_coords = SkyCoord(lon=hgs_lon_new * u.deg,
#                         lat=hgs_lat_new * u.deg,
#                         # radius=const.R_sun,
#                         obstime=flare_peak_times,
#                         frame='heliographic_stonyhurst')
# flare_coords = SkyCoord(Tx=hpc_x.values * u.arcsec, Ty=hpc_y.values * u.arcsec,obstime=flare_peak_times,
#      observer='earth', frame=frames.Helioprojective)
# ################# test block #################
# ################# test block #################
# ################# test block #################
...
marker_sizes = flare_scale_function(filtered_flares['GOES_fl_scls'].values)
...
solo_coords = SkyCoord(lon=filtered_flares['solo_position_lon'].values * u.deg,
                       lat=filtered_flares['solo_position_lat'].values * u.deg,
                       radius=filtered_flares['solo_position_AU_distance'].values * u.AU,
                       obstime=flare_peak_times,
                       frame='heliographic_stonyhurst')

earth_coords = SkyCoord(lon=filtered_flares['earth_position_lon'].values * u.deg,
                        lat=filtered_flares['earth_position_lat'].values * u.deg,
                        radius=filtered_flares['earth_position_AU_distance'].values * u.AU,
                        obstime=flare_peak_times,
                        frame='heliographic_stonyhurst')
...
filtered_flares['angles'] = angles
filtered_flares['angles_flare'] = angles_flare

# Separate data by flare class
angles_data = {
    2: filtered_flares[filtered_flares['numeric_class'] == 2]['angles'].values,
    3: filtered_flares[filtered_flares['numeric_class'] == 3]['angles'].values,
    4: filtered_flares[filtered_flares['numeric_class'] == 4]['angles'].values,
    5: filtered_flares[filtered_flares['numeric_class'] == 5]['angles'].values
}

angles_flare_data = {
    2: filtered_flares[filtered_flares['numeric_class'] == 2]['angles_flare'].values,
    3: filtered_flares[filtered_flares['numeric_class'] == 3]['angles_flare'].values,
    4: filtered_flares[filtered_flares['numeric_class'] == 4]['angles_flare'].values,
    5: filtered_flares[filtered_flares['numeric_class'] == 5]['angles_flare'].values
}
...
filtered_flares['Date Time (UT)'] = pd.to_datetime(filtered_flares['Date Time (UT)'])
...
filtered_flares.plot(x='Date Time (UT)', y=egs, ax=ax_flux,
